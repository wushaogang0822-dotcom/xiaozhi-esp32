 ...
 *   - 新增 PET_STAGE_EGG 阶段
 *   - XP 阈值调整为2天养成
 *   - 新增 pet_on_ai_message()：扫描 AI 回复中的 [XP:XXX] 码触发任务
 */
...
{
    if (xp < PET_XP_EGG_MAX)    return PET_STAGE_EGG;
...
static void _nvs_load(void)
{
    ...
    uint32_t val = 0;
    if (nvs_get_u32(h, NVS_XP,  &val) == ESP_OK) g_pet.saved.total_xp       = val;
    if (nvs_get_u32(h, NVS_DAY, &val) == ESP_OK) g_pet.saved.last_reset_day = val;
    uint8_t stage = 0;
    if (nvs_get_u8 (h, NVS_STG, &stage) == ESP_OK) g_pet.saved.stage = (PetStage)stage;
    nvs_close(h);
    ESP_LOGI(TAG, "Loaded: xp=%lu stage=%d", g_pet.saved.total_xp, g_pet.saved.stage);
}
...
void pet_game_save(void)
{
    ...
    nvs_set_u8 (h, NVS_STG, (uint8_t)g_pet.saved.stage);
    nvs_commit(h);
    nvs_close(h);
...
static void _daily_reset(void)
{
    ...
    if (today <= g_pet.saved.last_reset_day) return;
...
static void _auto_save_cb(TimerHandle_t t) { pet_game_save(); }
static void _tick_cb(TimerHandle_t t)      { pet_tick_time_events(); }
...
const PetState* pet_get_state(void) { return &g_pet; }

uint32_t pet_complete_task(TaskType task)
{
    if (task >= TASK_MAX) return 0;
    if (g_pet.tasks_done[task]) return 0;
    ...
    if (g_pet.saved.stage > old_stage) {
        ...
        g_pet.stage_up_flash = true;
        g_pet.stage_up_ticks = 8;   // 约1.6秒闪光
        ESP_LOGI(TAG, "STAGE UP! %d -> %d", old_stage, g_pet.saved.stage);
    } else {
        g_pet.mood            = PET_MOOD_HAPPY;
        g_pet.happy_ticks_left = 10;
    }

    ESP_LOGI(TAG, "Task %d +%lu XP, total=%lu, stage=%d",
             task, xp, g_pet.saved.total_xp, g_pet.saved.stage);
    return xp;
}
...
/**
 * 在 esp_sparkbot_board.cc 的 SetChatMessage("assistant", text) 里调用。
 * 扫描 AI 回复中的 [XP:XXX] 码，自动完成对应任务并加 XP。
 */
void pet_on_ai_message(const char* text)
{
    if (!text) return;

    struct { const char* code; TaskType task; } table[] = {
        {AI_CODE_WORD,  TASK_ENGLISH_WORD},
        {AI_CODE_SENT,  TASK_ENGLISH_SENT},
        {AI_CODE_POEM,  TASK_POETRY},
        {AI_CODE_BRUSH, TASK_BRUSH_TEETH},
        {AI_CODE_NOSE,  TASK_WASH_NOSE},
    };

    for (int i = 0; i < 5; i++) {
        if (strstr(text, table[i].code)) {
            uint32_t xp = pet_complete_task(table[i].task);
            if (xp > 0) {
                ESP_LOGI(TAG, "AI triggered task %d +%lu XP", table[i].task, xp);
                pet_display_show_task_done(table[i].task, xp);
            }
        }
    }
}
...
const char* pet_task_name(TaskType task)
{
    switch (task) {
        ...
        case TASK_ENGLISH_WORD: return "英语单词";
        case TASK_ENGLISH_SENT: return "英语短句";
        case TASK_POETRY:       return "背古诗";
        case TASK_BRUSH_TEETH:  return "刷牙";
        case TASK_WASH_NOSE:    return "洗鼻子";
        default:                return "任务";
    }
}
